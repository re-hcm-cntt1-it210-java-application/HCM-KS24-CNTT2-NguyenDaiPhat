...
public class AuthController {
  ...
  @GetMapping("/login") public String loginForm(Model model) { model.addAttribute("loginDto", new LoginDto()); return "auth/login"; }
  @PostMapping("/login") public String login(@Valid @ModelAttribute LoginDto loginDto, BindingResult br, HttpSession session, Model model) {
    ...
    if (userOpt.isEmpty()) { model.addAttribute("error", "Sai tai khoan hoac mat khau"); return "auth/login"; }
    var u = userOpt.get(); session.setAttribute("USER_ID", u.getId()); session.setAttribute("ROLE", u.getRole()); session.setAttribute("FULL_NAME", u.getFullName());
    return switch (u.getRole()) { case STUDENT -> "redirect:/student/schedule"; case LECTURER -> "redirect:/lecturer/sessions"; case ADMIN -> "redirect:/admin/equipments"; };
  }
  @GetMapping("/register") public String registerForm(Model model) { model.addAttribute("registerDto", new RegisterDto()); return "auth/register"; }
